package ccir2020MV.model.repository.classes;
	...
	public List<Activity> activitiesOnDate(String name, Date d) {
		...
		for (Activity a : activities)
			if (a.getName().equals(name))
                if (((a.getStart().getYear() == d.getYear() && a.getStart().getMonth() == d.getMonth() && a.getStart().getDate() == d.getDate()) ||
                        ( a.getDuration().getYear() == d.getYear() && a.getDuration().getMonth() == d.getMonth() && a.getDuration().getDate() == d.getDate()))
                        )
					result1.add(a);
		List<Activity> result = new LinkedList<Activity>();
		while (result1.size() > 0 )
